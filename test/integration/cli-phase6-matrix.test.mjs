...
async function writeAuthoredMap(path, width, height, data) {
	await writeFile(
		path,
		`${JSON.stringify({ width, height, data }, null, 2)}\n`,
		"utf8",
	);
}
...
describe("Phase 6 CLI integration matrix", () => {
	it("rejects invalid generate output-argument combination", async () => {
		...
		expect(result.stderr).toContain(
			"--output-dir is not valid in generate mode. Use --output-file.",
		);
	});
	...
	it("covers debug happy path without --debug-output-file", async () => {
		...
		await expect(
			stat(join(outputDir, "debug-manifest.json")),
		).resolves.toBeDefined();
		await expect(
			stat(join(outputDir, "topography.json")),
		).resolves.toBeDefined();
		await expect(
			stat(join(outputDir, "hydrology.json")),
		).resolves.toBeDefined();
		await expect(stat(join(outputDir, "ecology.json"))).resolves.toBeDefined();
		await expect(
			stat(join(outputDir, "navigation.json")),
		).resolves.toBeDefined();
	});

	it("covers debug happy path from --input-file", async () => {
		const dir = await makeTempDir();
	...
	it("rejects --input-file outside debug mode", async () => {
		const result = await runCli([
			"generate",
			"--seed",
			"1",
		...
		expect(result.code).toBe(2);
		expect(result.stderr).toContain(
			"--input-file is only valid in debug mode.",
		);
	});
