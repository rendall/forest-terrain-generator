...
const HYDROLOGY_INSPECTOR_CLI_ENTRY = resolve(process.cwd(), "src/cli/hydrology-inspector.ts");
const tempDirs = [];
...
async function makeTempDir() {
	const dir = await mkdtemp(join(tmpdir(), "forest-terrain-generator-hydrology-inspector-cli-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("hydrology-inspector CLI wiring", () => {
	it("returns routed [x,y] path as JSON", async () => {
		...
		const result = await runCli(HYDROLOGY_INSPECTOR_CLI_ENTRY, [
			"--input-json",
			sourceFile,
			"--x",
			"2",
	...
	it("prefers tie-breaks by direction-of-flow then toward map center then tile index", async () => {
		const dir = await makeTempDir();
		const sourceFile = join(dir, "source-hydrology-inspector-tiebreak.json");
		await writeFile(
			sourceFile,
			`${JSON.stringify(
				{
					...
					tiles: [
						...
						{ x: 2, y: 0, topography: { h: 0.9, r: 0, v: 0 } },
						{ x: 0, y: 1, topography: { h: 0.4, r: 0, v: 0 } },
						{ x: 1, y: 1, topography: { h: 0.8, r: 0, v: 0 } },
						{ x: 2, y: 1, topography: { h: 0.7, r: 0, v: 0 } },
						{ x: 0, y: 2, topography: { h: 0.9, r: 0, v: 0 } },
						{ x: 1, y: 2, topography: { h: 0.7, r: 0, v: 0 } },
						{ x: 2, y: 2, topography: { h: 0.9, r: 0, v: 0 } },
					],
				},
				null,
				2,
