...
describe("Phase 3 hydrology sanity (red tests before implementation)", () => {
	...
	it("uses deterministic hash-based tie-break for equal-drop candidates", async () => {
		...
		const h = new Float32Array([1.2, 1.2, 1.2, 1.2, 1.0, 0.8, 1.2, 0.8, 1.2]);
		const params = {
		...
		const expectedDir =
			tied[Number(hydrology.tieBreakHash64(seed, 1, 1) % BigInt(tied.length))];
	...
	it("produces acyclic flow-direction graphs", async () => {
		...
		const h = new Float32Array([
			1.0, 0.9, 0.8, 0.7, 1.0, 0.9, 0.8, 0.7, 1.0, 0.9, 0.8, 0.7, 1.0, 0.9, 0.8,
			0.7,
		]);
	...
	it("is deterministic across repeated full hydrology runs", async () => {
		...
		const lakeA = hydrology.deriveLakeMask(
			shape,
			landform,
			slopeMag,
			faNA,
		...
		const streamA = hydrology.deriveStreamMask(
			shape,
			lakeA,
			faNA,
			slopeMag,
		...
		const wcA = hydrology.classifyWaterClass(
			shape,
			lakeA,
			streamA,
		...
		const lakeB = hydrology.deriveLakeMask(
			shape,
			landform,
			slopeMag,
			faNB,
		...
		const streamB = hydrology.deriveStreamMask(
			shape,
			lakeB,
			faNB,
			slopeMag,
		...
		const wcB = hydrology.classifyWaterClass(
			shape,
			lakeB,
			streamB,
			new Uint8Array(shape.size),
