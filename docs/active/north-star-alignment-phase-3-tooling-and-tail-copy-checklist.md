...
| [ ] | NS3-TEST-02 | Test expectations still encode legacy hydrology-copy assumptions (`lakeMask`/schema-centric wording). | `test/integration/cli-command-wiring.test.mjs:277-284` | Contradicts hydrology-handoff direction that `lakeMask` is legacy. | Update expectation copy/comments to treat `lakeMask` as compatibility field where still present. |
| [ ] | NS3-PATH-01 | Misspelled active-doc reference appears in working habits (`hydrologyHandloff`). | user/dev workflow references; canonical file is `docs/active/hydrology-handoff.md` | Increases drift and lookup errors for source-of-truth handoff doc. | Add a short "canonical active docs" section in README and use only `hydrology-handoff.md` spelling in docs. |
