...
- [ ] [pipeline] `SPI-03` Implement `strict_local` sink mode in `deriveHydrology`, with sink termination at local minima and no spill continuation, using the same shared FD builder as `overflow_guided` (sink policy only changes how unresolved sinks are handled).
