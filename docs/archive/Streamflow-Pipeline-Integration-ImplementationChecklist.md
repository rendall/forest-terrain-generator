...
- [x] [governance] `SPI-15` Author a dedicated ADR immediately before Phase D to approve public hydrology schema exposure and migration/versioning policy.
- [x] [invariant] `SPI-16` Enforce overflow edge invariant in hydrology implementation/docs: `spillFromTileId` is in-basin and `spillToTileId` is adjacent and outside basin; do not infer overflow from `spillTileId` membership.
- [x] [cli] `SPI-17` Ensure `hydrology-inspector` can recompute hydrology maps from envelopes that do not contain precomputed hydrology outputs, while preferring pipeline maps when available.
...
- Verified complete against current implementation: `SPI-01` through `SPI-17` (all items complete).
- Verification run:
  - `npm test` -> passed (`16` files, `65` tests).
  - `npm run typecheck` -> still fails in pre-existing unrelated files (`run-describe`, `run-los`, `run-stream`, `description`); no new type errors introduced in checklist-touched files.
