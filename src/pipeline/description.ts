...
type Visibility = "short" | "medium" | "long";

type WaterClass = "none" | "marsh" | "stream" | "lake";

type Biome = "spruce_swamp" | "pine_heath" | "mixed_forest";

type Landform = "ridge" | "basin" | "plain" | "flat" | "low_rise";
...
const LANDFORM_PHRASES: Record<Landform, string[]> = {
  ridge: [
...
const BIOME_PHRASES: Record<Biome, string[]> = {
  spruce_swamp: [
...
function chooseSlopeBand(slopeStrength: number): "gentle" | "noticeable" | "steep" | null {
...
function chooseDirectional(input: DescriptionTileInput): Direction | null {
  ...
  let bestDiagonal: { dir: Direction; score: number; water: WaterClass } | null = null;
...
function renderDirectionalSentence(input: DescriptionTileInput, seedKey: string): string | null {
  ...
  if (neighbor.water === "lake") {
    const phrase = pickDeterministic(LAKE_PHRASES, `${seedKey}:dir:lake:${direction}`);
    return `${connector}, ${phrase}.`;
  }
  if (neighbor.water === "stream") {
    const phrase = pickDeterministic(STREAM_PHRASES, `${seedKey}:dir:stream:${direction}`);
    return `${connector}, ${phrase}.`;
  }
...
function chooseObstacle(input: DescriptionTileInput): Obstacle | null {
  ...
  return [...input.obstacles].sort((a, b) => obstaclePriority(a) - obstaclePriority(b))[0] ?? null;
}

function renderObstacleSentence(input: DescriptionTileInput, seedKey: string): string | null {
  const obstacle = chooseObstacle(input);
  if (!obstacle) {
    return null;
  }
  if (obstacle === "root_tangle") {
    return pickDeterministic(ROOT_TANGLE_PHRASES, `${seedKey}:obstacle:root_tangle`);
  }
...
export function generateRawDescription(input: DescriptionTileInput, seedKey: string): DescriptionResult {
  ...
  const landformSentence = pickDeterministic(
    LANDFORM_PHRASES[input.landform],
    `${seedKey}:landform:${input.landform}`
  );
  const biomeSentence = pickDeterministic(BIOME_PHRASES[input.biome], `${seedKey}:biome:${input.biome}`);
  ...
  if (shouldMentionWater(input)) {
    if (input.standingWater) {
      sentences.push({
        slot: "hydrology",
        text: pickDeterministic(STANDING_WATER_PHRASES, `${seedKey}:hydrology:standing`)
      });
    }
  }
  ...
  if (directionalSentence) {
    const existingDirectional = sentences.some((sentence) => sentence.slot === "directional");
