...
function readNormalizeParams(
	...
	if (typeof enabledRaw !== "boolean") {
		throw new Error(
			`Missing or invalid boolean params value "${key}.normalize.enabled".`,
		);
	}
	...
	const lowerQ =
		lowerQRaw === undefined
			? 0.02
			: expectNumber(lowerQRaw, `${key}.normalize.lowerQ`);
	const upperQ =
		upperQRaw === undefined
			? 0.98
			: expectNumber(upperQRaw, `${key}.normalize.upperQ`);
	if (
		lowerQ < 0 ||
		lowerQ > 1 ||
		upperQ < 0 ||
