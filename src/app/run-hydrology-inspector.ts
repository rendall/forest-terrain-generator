...
export async function writeHydrologyInspectorOverlayPpm(
...
export async function runHydrologyInspectorTrace(
	...
	const chooseLowestDownhillNeighbor = (
		...
		const currentCenterDx = centerX - currentX;
		const currentCenterDy = centerY - currentY;
		...
		for (const n of neighbors) {
			...
			const nFlowScore = (nx - currentX) * currentCenterDx + (ny - currentY) * currentCenterDy;
			const bestFlowScore =
				(bestX - currentX) * currentCenterDx + (bestY - currentY) * currentCenterDy;
			if (nFlowScore > bestFlowScore) {
				best = n;
				continue;
			}
			if (nFlowScore < bestFlowScore) {
			...
			const nCenterDist2 = (centerX - nx) ** 2 + (centerY - ny) ** 2;
			const bestCenterDist2 = (centerX - bestX) ** 2 + (centerY - bestY) ** 2;
			if (nCenterDist2 < bestCenterDist2) {
				best = n;
				continue;
			}
			if (nCenterDist2 > bestCenterDist2) {
...
export async function runHydrologyInspector(
	request: StreamRequest,
): Promise<StreamStep[]> {
	const result = await runHydrologyInspectorTrace(request);
	return result.path;
}
