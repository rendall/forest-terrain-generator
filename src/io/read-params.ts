...
const PARAMS_VALIDATION_SCHEMA: JsonObject = {
	...
	vegVarianceStrength: (APPENDIX_A_DEFAULTS.vegVarianceNoise as JsonObject)
		.strength,
};
...
function noisePath(
	noiseKey: "heightNoise" | "roughnessNoise" | "vegVarianceNoise",
	key: string,
): string {
	return `params.${noiseKey}.${key}`;
}

function validateNoiseNormalizeParams(
	params: JsonObject,
	noiseKey: "heightNoise" | "roughnessNoise" | "vegVarianceNoise",
): void {
	if (!isObject(params[noiseKey])) {
		return;
	}

	const noise = params[noiseKey] as JsonObject;
	const normalize = noise.normalize;
	...
	if (!isObject(normalize)) {
		throw new InputValidationError(
			`Invalid params value "params.${noiseKey}.normalize". Expected an object.`,
		);
	}
	...
	expectOptionalBoolean(
		value.enabled,
		noisePath(noiseKey, "normalize.enabled"),
	);
	expectOptionalEnum(value.mode, noisePath(noiseKey, "normalize.mode"), [
		"minmax",
		"quantile",
	]);
	expectOptionalRangeNumber(
		value.lowerQ,
		noisePath(noiseKey, "normalize.lowerQ"),
		0,
		1,
	);
	expectOptionalRangeNumber(
		value.upperQ,
		noisePath(noiseKey, "normalize.upperQ"),
		0,
		1,
	);
	const lowerQ = finiteNumberOrUndefined(value.lowerQ);
	const upperQ = finiteNumberOrUndefined(value.upperQ);
	if (lowerQ !== undefined && upperQ !== undefined && lowerQ >= upperQ) {
		throw new InputValidationError(
			`Invalid params value "${noisePath(noiseKey, "normalize.lowerQ")}/${noisePath(noiseKey, "normalize.upperQ")}". Expected lowerQ < upperQ.`,
		);
	}
}
...
function validateLakeCoherenceParams(params: JsonObject): void {
	...
	expectOptionalEnum(value.microLakeMode, lakeCoherencePath("microLakeMode"), [
		"merge",
		"remove",
		"leave",
	]);
	expectOptionalBoolean(
		value.bridgeEnabled,
		lakeCoherencePath("bridgeEnabled"),
	);
	expectOptionalNonNegativeInteger(
	...
	expectOptionalBoolean(
		value.repairSingletons,
		lakeCoherencePath("repairSingletons"),
	);
	expectOptionalBoolean(
...
function normalizeLegacyHydrologyAliases(params: JsonObject): void {
	...
	const sourceAccumMin = finiteNumberOrUndefined(
		streamThresholds.sourceAccumMin,
	);
...
function validateHydrologyPipelineParams(params: JsonObject): void {
	...
	const hydrology = params.hydrology as JsonObject;
	expectOptionalEnum(hydrology.sinkMode, hydrologyPath("sinkMode"), [
		"strict_local",
		"overflow_guided",
	]);
	expectOptionalNonNegativeInteger(
		hydrology.faThreshold,
		hydrologyPath("faThreshold"),
	);
	expectOptionalRangeNumber(
		hydrology.faQuantileThreshold,
		hydrologyPath("faQuantileThreshold"),
		0,
		1,
	);
}
...
export async function readParamsFile(
	...
	const isWrapperMode =
		"params" in parsed ||
		wrapperKeys.some((key) => key !== "params" && key in parsed);
	...
	if (isWrapperMode) {
		for (const key of Object.keys(parsed)) {
			if (!wrapperKeys.includes(key)) {
				...
				const hint = suggestion ? ` Did you mean "${suggestion}"?` : "";
				throw new InputValidationError(
					`Unknown params wrapper key "${key}".${hint}`,
				);
			}
		}
	}
