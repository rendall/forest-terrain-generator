...
function assertObject(
	...
	if (typeof value !== "object" || value === null || Array.isArray(value)) {
		throw new InputValidationError(
			"Authored map file must contain a JSON object.",
		);
	}
}
...
function assertInteger(value: unknown, key: string): asserts value is number {
	if (typeof value !== "number" || !Number.isInteger(value) || value <= 0) {
		throw new InputValidationError(
			`Authored map key "${key}" must be a positive integer.`,
		);
	}
}
...
function validateExpectedShape(
	...
	if (
		shape.width !== expectedShape.width ||
		shape.height !== expectedShape.height
	) {
		throw new ShapeValidationError(
